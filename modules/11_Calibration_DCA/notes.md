...
- **与研究工作的联系**：在 EcMurJ 虚拟筛选中，化合物活性预测的概率值需要可信校准，以决定后续实验验证的优先级；DCA 可用于确定"在什么概率阈值下值得进行分子对接实验"。在宏基因组研究中，疾病风险预测模型的临床应用需要 DCA 证明其在相关阈值下有净获益。校准分析对于需要概率值（而非仅分类标签）的下游分析尤为关键，如风险分层、资源分配等。
