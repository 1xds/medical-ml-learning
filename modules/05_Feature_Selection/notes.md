...
- **与研究工作的联系**：在 EcMurJ 虚拟筛选中，分子描述符集（如 PaDEL 描述符）通常包含数千个特征，其中高度共线（如多种疏水性描述符）和噪声特征占据多数。Boruta（统计严谨）+ RF Importance（计算可承受）的组合策略适合处理该类数据。在宏基因组研究中，微生物物种丰度矩阵的高维稀疏特性更适用 LASSO 或 Elastic Net——L1 正则化可同时完成特征选择和模型拟合。
