# Resources

> 只放链接，不复制内容。

---

## 核心教程

- **ml4health 仓库**: 教程原始代码与讲义
- **数据集下载**: 提取码 JQ85

---

## 推荐参考书

- 《统计学习方法》李航
- 《Python机器学习基础教程》 Andreas C. Muller
- 《Interpretable Machine Learning》 Christoph Molnar

---

## 在线资源

### EDA 与统计
- [Seaborn 官方文档](https://seaborn.pydata.org/)
- [SciPy 统计检验](https://docs.scipy.org/doc/scipy/reference/stats.html)

### 特征工程
- [scikit-learn Pipeline](https://scikit-learn.org/stable/modules/compose.html)
- [Feature-engine](https://feature-engine.trainindata.com/)

### 建模评估
- [scikit-learn 模型评估](https://scikit-learn.org/stable/modules/model_evaluation.html)
- [imbalanced-learn](https://imbalanced-learn.org/)

### 可解释性
- [SHAP 官方文档](https://shap.readthedocs.io/)
- [InterpretML](https://interpret.ml/)

### 因果推断
- [EconML](https://econml.azurewebsites.net/)
- [DoubleML](https://docs.doubleml.org/)

### 影像组学
- [PyRadiomics](https://pyradiomics.readthedocs.io/)

### 基因组学
- [TCGA 数据门户](https://portal.gdc.cancer.gov/)
- [cBioPortal](https://www.cbioportal.org/)

---

## 关键论文

- Kohga H et al. (2025) Sci Adv 11, eady8083
- Lundberg SM, Lee SI. (2017) NeurIPS - SHAP
- Chernozhukov V et al. (2018) Econometrics Journal - DML

---

## 工具速查

| 工具 | 用途 | 模块 |
|------|------|------|
| pandas | 数据处理 | 01-22 |
| scikit-learn | 机器学习 | 03-11 |
| SHAP | 模型解释 | 12-17 |
| EconML/DoubleML | 因果推断 | 18 |
| PyRadiomics | 影像组学 | 19-20 |
| matplotlib/seaborn | 可视化 | 全部 |
